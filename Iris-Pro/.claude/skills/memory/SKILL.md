...
Manages the 3-tier persistent memory system: mem0 + Pinecone, session logs, and MEMORY.md.
...
3. **Long-Term Memory** — mem0 + Pinecone, automatic extraction + semantic search
...
1. **mem0 `get_all()` / `delete_all()` fallback**: Some mem0 vector-store backends have intermittent bugs in these calls. The list and delete scripts fall back to the local history DB when that happens.
